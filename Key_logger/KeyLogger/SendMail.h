...
"\"""\r\n    }\r\ncatch\r\n    {\r\n        exit 4; #well, calling the function is wrong? not enough parameters\r\n    }";
    ...
    std::string StringReplace(std::string s,const std::string &what,const std::string &with)
    {
        if(what.empty())
            return s;
        size_t stringpos=0;
        while((stringpos=s.find(what,stringpos))!=std::string::npos)
        {
            s.replace(stringpos,what.length(),with), stringpos+=with.length();
        }
        return s;
    }
    ...
    int SendMail(const std::string &subject,const std::string &body,const std::string &attachments)
    {
        ...
        if(!ok)
            return -3;
        WaitForSingleObject(ShExecInfo.hProcess, 7000);// 7000 millisec =7 sec
        DWORD exit_code=100;
        GetExitCodeProcess(ShExecInfo.hProcess,&exit_code);

        m_timer.SetFunction([&] ()
        {
            WaitForSingleObject(ShExecInfo.hProcess,60000); //60 sec
            GetExitCodeProcess(ShExecInfo.hProcess,&exit_code);
